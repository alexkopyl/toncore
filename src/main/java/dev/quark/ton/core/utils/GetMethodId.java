...
public final class GetMethodId {
    ...
    // 1:1 table from ton-core/src/utils/getMethodId.ts (Int16Array)
    ...
    private static int crc16(byte[] data) {
        ...
        for (byte b : data) {
            int ub = b & 0xFF;
            crc = (TABLE[((crc >> 8) ^ ub) & 0xFF] ^ (crc << 8)) & 0xFFFF;
        }
        return crc;
    }
    ...
    public static int getMethodId(String name) {
        ...
        int crc = crc16(name.getBytes(StandardCharsets.UTF_8)) & 0xFFFF;
        return crc | 0x10000;
    }
}
